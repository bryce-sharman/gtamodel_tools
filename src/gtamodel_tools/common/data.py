""" Functions that assist data processing required throughout the package.

Much of this code was provided by WSP Canada.
...
def round_to_totals(df: pd.DataFrame, rounding_def: dict) -> pd.DataFrame:
    """ Rounds fields in a dataframe to respect totals.

    This approach will round all zones based on the decimal
    component. Whereas a typical rounding function will use a threshold
    of 0.5 to determine whether to round down or round up, this function
    search for a threshold between 0.0001 and 0.9999, which will cause the 
    sum of the rounded values to match the total. 
    ...
    rounding_def: dict
        Defines rounding. Expected to be structured as follows:
        - key: column to be rounded
        - values: int | float. 
            Total value after rounding.

    Returns:
    pd.DataFrame
        dataframe with rounding values.
    """
    break_threshold = 1e-7
    max_iterations = 50
    df2 = df.copy()
    for key, total in rounding_def.items():
        if key not in df.columns:
            raise KeyError(f"Invalid column: {key}.")
        if not isinstance(total, int):
            raise AttributeError(
                'This method expects total argument to be an integer.')
        # Check that it is possible to reach the totals
        # This max delta assumes evenly distributed fractions with
        # a max of 1 or min of 0, making the average change of 0.5.
        current_sum = df[key].sum()
        delta = fabs(total - current_sum)
        max_delta = len(df) * 0.5
        if max_delta < delta:
            raise AttributeError(
                f"Total {total} cannot be achieved by rounding column {key}."
            )
        threshold = 0.5
        threshold_delta = 0.25
        iter = 0

        # Remove exact duplicates by perturbing any elements
        while True:
            fltr = df[key].duplicated()
            if fltr.sum() == 0:
                break
            df[fltr, key] = df[fltr, key] + 0.01

        while True:
            s = df[key].copy()
            s_floor = s.astype(np.int64)
            # This line is to make sure that we round negative
            # numbers down instead of towards 0, which is what the 
            # integer casting does
            s_floor.loc[s < 0] = s_floor.loc[s < 0] - 1
            s_ceil = s_floor + 1
            diff = s - s_floor
            round_down = diff < threshold
            round_up = diff >= threshold
            s.loc[round_down] = s_floor[round_down]
            s.loc[round_up] = s_ceil[round_up]
            new_sum = s.sum()
            if threshold >= (1.0 - break_threshold) or \
                    threshold < break_threshold:
                raise RuntimeError(
                    f"Threshold exceeds allowed bounds. Cannot reach rounding, "
                    f" target in column {key}.")
            elif iter > max_iterations:
                # Could not converge but it's within range, just exit and
                # call this close enough.
                break
            elif new_sum > total:
                threshold += threshold_delta
                threshold_delta = 0.5 * threshold_delta
            elif new_sum < total:
                threshold -= threshold_delta
                threshold_delta = 0.5 * threshold_delta
            else:
