...
class SyntheticPopulation():
    ...
    """
    def  __init__(self, model_version: ModelVersion, households_fp: Union[str, Path], persons_fp: Union[str, Path]):
        """ Initialize SyntheticPopulation class. 

        Args:
            model_version: Travel demand model version for which this synthetic population input is intended.
    ...
    def _read_synthetic_households(self, model_version: ModelVersion, households_fp: Union[str, Path]) -> None:
    ...
    def _read_synthetic_persons(self, model_version, persons_fp: Union[str, Path]) -> None:
    ...
    def summarize_households(
        ...
        """ Performs a spatial aggregation of synthetic population households, allowing optional filters.
        
        Args:
            home_sa: Spatial aggregation on home zone. 
                If None, then summaries will be reported at the home zone level.
            weight_expr: Value to be aggregated. This is an expression that will be evaluatated using pandas .eval.
            ...
            hhld_fltr_expr: filter expression, will be used by pandas.eval to filter household data using their 
                attributes. If None then no filter is applied using household attributes.
        ...
        return sa.summarize_table_with_spatial_aggregation(hhlds_df, weight_expr, enums_sp.HHLD_HOMEZONE_COL, home_sa, crosstabs, crosstab_segments)


    def summarize_persons(
            self,
            weight_expr: str = enums_sp.PERS_WEIGHT_COL,
            home_sa: Union[Type[sa.SpatialAggregator], None, False] = False,
            work_sa: Union[Type[sa.SpatialAggregator], None, False] = False,
            school_sa: Union[Type[sa.SpatialAggregator], None, False] = False,
            crosstabs: Union[str, List[str], None] = None,
            crosstab_segments: Union[Dict, List[Dict], None] = None,
            hhld_fltr_expr: Optional[str] = None,
            pers_fltr_expr: Optional[str] = None,
        ) -> pd.DataFrame:
        """ Performs a spatial aggregation of synthetic population persons, allowing optional filters.

        Even though there are only a population and household file and it is possible to perform a single
        merge on these file, this method uses the same approach as is anticipated for the microsim data. 
        
        This involves performing independent filters of the households and persons file before merging
        data together, if this is required. The purpose to do this is to save memory from the 
        merged data, which admittedly is not a major problem in this case, but is expected to become one for
        the microsim data.
        ...
        Args:
            ...
            school_sa: Spatial aggregation on school zone.  
                If None, then summaries will be reported at the school zone level.
            ...
            hhld_fltr_expr: filter expression, will be used by pandas.eval to filter household data using their 
                attributes. If None then no filter is applied using household attributes.
            pers_fltr_expr: filter expression, will be used by pandas.eval to filter persons data using their 
                attributes. If None then no filter is applied using persons attributes.
                
        Returns:
            pandas.DataFrame with the summary data.
            
        """
        # Any aggregation definition that is not explicitly False (i.e. None or sa.SpatialAggregator) will be created
        ...
        if aggr_home + aggr_work + aggr_school < 1:
            raise ValueError("At least one spatial aggregation must be defined.")
        ...
        if home_sa or hhld_fltr_expr or has_hhld_crosstab_col:
            ...
            # Dropping the households weight as we won't use that for a persons summary 
            # and name conflicts with persons weight
            hhlds_df = data.apply_dataframe_filter(self._households, hhld_fltr_expr)
            hhlds_df = hhlds_df.drop(enums_sp.HHLD_WEIGHT_COL, axis=1)
            pers_df = pers_df.merge(hhlds_df, how="inner", on=enums_sp.HHLD_ID_COL)

        return sa.summarize_table_with_spatial_aggregation(
            df=pers_df, 
            values=weight_expr, 
            zones=[enums_sp.HHLD_HOMEZONE_COL, enums_sp.PERS_WORKZONE_COL, enums_sp.PERS_SCHOOLZONE_COL],
